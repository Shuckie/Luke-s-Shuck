...
var color:CustomShader;
var perspective:CustomShader;

var camShader:FlxCamera = new FlxCamera();
...
function create() {
    ...
    if (FlxG.save.data.language == "espanol")
        vocals = FlxG.sound.load(Options.streamedVocals ? Assets.getMusic(Paths.voices(SONG.meta.name, "Espanol")) : Paths.voices(SONG.meta.name, "Espanol"));
    
    if (FlxG.save.data.language == "portuguese"){
        vocals = FlxG.sound.load(Options.streamedVocals ? Assets.getMusic(Paths.voices(SONG.meta.name, "Portuguese")) : Paths.voices(SONG.meta.name, "Portuguese"));
    }
...
function postCreate() {
    ...
    doorVid.scale.set(0.75, 0.75);
    doorVid.setPosition(-317, -180);

    add(lyricVid); 
    lyricVid.load(Assets.getPath(Paths.file('videos/shucks-lyrics.mkv')));
    lyricVid.scale.set(0.68, 0.68);
    lyricVid.setPosition(-317, -180);
    lyricVid.scrollFactor.set();

    add(bitchVid);
    bitchVid.load(Assets.getPath(Paths.file('videos/shucks-youbitch.mkv')));
...
function postPostCreate() {
    ...
    strumLines.members[3].characters[0].alpha = strumLines.members[0].characters[6].alpha = strumLines.members[0].characters[5].alpha = 0;
...
function stepHit(_:Int) {
    switch (_) {
        ...
        case 72:
            remove(doorVid);
            camHUD.alpha = 0;
            if (Options.gameplayShaders && FlxG.save.data.iridaPerspectiveWarp) {
                    perspective = new CustomShader('perspective');
                    camGame.addShader(perspective);
                    perspective.Intensity = -0.25;
                }
        ...
        case 304:
    FlxTween.tween(camHUD, {alpha: 0, 'scroll.y': 0}, (Conductor.stepCrochet / 1000) * 16, {ease: FlxEase.circInOut});
    FlxTween.tween(camNotes, {'scroll.y': 0, alpha: 0}, (Conductor.stepCrochet / 1000) * 16, {ease: FlxEase.circInOut});
        case 452: 
        ...
        case 454:
            if (Options.gameplayShaders && FlxG.save.data.iridaColorCorrect) {
                    FlxTween.num(200,  350, (Conductor.stepCrochet / 1000) * 32, {ease: FlxEase.quartOut}, val -> ass.hue = val); 
                    FlxTween.num(-80,  0, (Conductor.stepCrochet / 1000) * 32, {ease: FlxEase.quartOut}, val -> ass.brightness = val);
                    FlxTween.num(60,  -10, (Conductor.stepCrochet / 1000) * 32, {ease: FlxEase.quartOut}, val -> ass.saturation = val);
                    FlxTween.num(80,  30, (Conductor.stepCrochet / 1000) * 32, {ease: FlxEase.quartOut}, val -> ass.contrast = val);
                }
        ...
        case 1884:
            ...
            if (Options.gameplayShaders && FlxG.save.data.iridaRoseEffects) {
                    FlxTween.num(0,  1, (Conductor.stepCrochet / 1000) * 16, {ease: FlxEase.quartOut}, val -> color.b = val);
                    FlxTween.num(0,  0.4, (Conductor.stepCrochet / 1000) * 16, {ease: FlxEase.quartOut}, val -> color.intensity = val);
                    FlxTween.num(0,  0.2, (Conductor.stepCrochet / 1000) * 16, {ease: FlxEase.quartOut}, val -> color.threshold = val);
                    FlxTween.num(0,  0.25, (Conductor.stepCrochet / 1000) * 16, {ease: FlxEase.quartOut}, val -> stars.Opacity = val);
                } 
        case 2144:
        ...
        case 2400:
            ...
            if (Options.gameplayShaders && FlxG.save.data.iridaRoseEffects) {
                    FlxTween.num(1,  0, (Conductor.stepCrochet / 1000) * 4, {ease: FlxEase.quartOut}, val -> color.b = val);
                    FlxTween.num(0.4,  0, (Conductor.stepCrochet / 1000) * 4, {ease: FlxEase.quartOut}, val -> color.intensity = val);
                    FlxTween.num(0.2,  0, (Conductor.stepCrochet / 1000) * 4, {ease: FlxEase.quartOut}, val -> color.threshold = val);
                    FlxTween.num(0.25,  0, (Conductor.stepCrochet / 1000) * 16, {ease: FlxEase.quartOut}, val -> stars.Opacity = val);
                } 
        ...
        case 2656:
            ...
            strumLines.members[0].characters[6].alpha = 1;
        ...
        case 3889:
            ...
            strumLines.members[0].characters[6].alpha = 0;
            camGame.stopFade();
        ...
        case 4136:
            executeEvent({
                name: "Camera Position",
                params: [21350, 657,true,8,"expo","Out",falses]
            }); 
        ...
        case 4396:
            ...
            strumLines.members[0].characters[2].alpha = strumLines.members[0].characters[3].alpha = strumLines.members[0].characters[4].alpha = strumLines.members[1].characters[2].alpha = 0;
            strumLines.members[1].characters[3].alpha = strumLines.members[0].characters[5].alpha = 1;
            
            var origX = strumLines.members[1].characters[3].x;
            var origY = strumLines.members[1].characters[3].y;
            var origScale = strumLines.members[1].characters[3].scale;

            trace("Original X: " + origX + " Original Y: " + origY);
            trace("Original Scale: " + origScale);

            var origLEGSX = stage.getSprite("run_legsmarv").x;
            var origLEGSY = stage.getSprite("run_legsmarv").y;
            var origLEGSScale = stage.getSprite("run_legsmarv").scale;

            trace("Original LEGS X: " + origLEGSX + " Original LEGS Y: " + origLEGSY);
            trace("Original LEGS Scale: " + origLEGSScale);

            strumLines.members[1].characters[3].x = 1770;
            strumLines.members[1].characters[3].y = 650;
            strumLines.members[1].characters[3].scale.set(3.3, 3.3);

            stage.getSprite("run_legsmarv").x = 1770;
            stage.getSprite("run_legsmarv").y = 505;
            stage.getSprite("run_legsmarv").scale.set(3.3, 3.3);
        ...
        case 4416:
            ...
            //strumLines.members[0].characters[5].animation.finishCallback = (name:String) -> if (name == "intro") stage.getSprite("run_legsdetg").alpha = 1;
            strumLines.members[0].characters[5].playAnim("intro", true);
            stage.getSprite("run_bg").playAnim("intro", true);

        case 4420:
            
            strumLines.members[0].characters[5].animation.finishCallback = (name:String) -> if (name == "intro") stage.getSprite("run_legsdetg").alpha = strumLines.members[0].characters[4].alpha = 1;
            strumLines.members[0].characters[5].animation.finishCallback = (name:String) -> if (name == "intro") strumLines.members[0].characters[5].alpha = 0;
            //strumLines.members[0].characters[5].playAnim("intro", true);
            
        case 4426:
            FlxTween.tween(strumLines.members[1].characters[3], {x: 570, y: 150, 'scale.x': 0.7, 'scale.y': 0.7}, 0.3, {ease: FlxEase.circInOut});
            FlxTween.tween(stage.getSprite("run_legsmarv"), {x: 425, y: -105, 'scale.x': 0.7, 'scale.y': 0.7}, 0.3, {ease: FlxEase.circInOut});
        case 4428:
            strumLines.members[0].characters[5].alpha = 0;    
            stage.getSprite("run_legsdetg").alpha = strumLines.members[0].characters[4].alpha = 1;
...
function update(_) {
    starsTimer += _ / 2;
    if (stars != null) stars.iTime = starsTimer;
}
