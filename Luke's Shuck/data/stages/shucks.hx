...
function beatHit(curBeat:Int) {
    if (curBeat % 1 == 0 && theStage == "run_") {
        walkDown = !walkDown;

        var char0TargetY = (walkDown && StringTools.startsWith(strumLines.members[0].characters[4].getAnimName(), "sing") ? -35 : -50);
        var char1TargetY = (walkDown && StringTools.startsWith(strumLines.members[1].characters[3].getAnimName(), "sing") ? 165 : 150);

        FlxTween.cancelTweensOf(strumLines.members[0].characters[4]);
        FlxTween.cancelTweensOf(strumLines.members[1].characters[3]);

        FlxTween.tween(strumLines.members[0].characters[4], {y: char0TargetY}, 0.2, {ease: FlxEase.bounceOut});
        FlxTween.tween(strumLines.members[1].characters[3], {y: char1TargetY}, 0.2, {ease: FlxEase.bounceOut});
    }
}
*/
